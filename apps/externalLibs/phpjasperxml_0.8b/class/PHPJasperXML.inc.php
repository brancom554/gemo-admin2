...
//### End of modification
    ...
    public function default_handler($xml_path) {
        foreach($xml_path as $k=>$out) {
         //    echo "<br>text field in process<br>><pre>";print_r($out);   //         exit;
            switch($k) {
                case "staticText":
            ...
            case "ellipse":
                    ...
                    break;
	         case "textField":
	//         	 echo "<br>text dield in process<br>><pre>";print_r($out);	 //        	exit;
...
//### UTF-8 characters, a must for me.
		...
		//echo "<br>Data printed ==> ";print_r($txtEnc);
	...
	//	echo "<br>pointer L370 / <br><pre>";print_r($this->pointer);
...
//        $b=$data->subreportParameter;
    ...
    public function transferDBtoArray($host,$user,$password,$db_or_dsn_name,$cndriver="mysql") {
      ...
      if(!$this->connect($this->iniObj->DbHost,$this->iniObj->DbUser,$this->iniObj->DbPass,$this->iniObj->DbName,$cndriver))  //connect database
      {
...
//      echo "<br> sql = <pre><br>".$this->sql;
      $result = $this->sqlData->fetchData($this->sql,$this->arrayParameter);
//                echo "<br> results  =><pre> ";print_r($result);
      foreach($result['data'] as $k=>$v) {
        foreach($this->arrayfield as $out) {

          $this->arraysqltable[$this->m]["$out"]=$result['data'][$k]->{$out};


        }
        $this->m++;
      }
  //      echo "<br> value M => ";print_r($this->m);
   //   echo "<br>php jasper after foreach <br><pre>";print_r($this->arraysqltable);exit;
    }
...
//        	         echo "<br>array band / <pre>";        print_r($this->arrayPageSetting);

            if($this->pdflib=="TCPDF") {
            //  $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
                if($this->arrayPageSetting["orientation"]=="P")
                    $this->pdf=new TCPDF($this->arrayPageSetting["orientation"],'pt',
                      array($this->arrayPageSetting["pageWidth"],$this->arrayPageSetting["pageHeight"])
                      );
                else
        ...
        // echo "<br>array band / <pre>";        print_r($this->arrayband);
        foreach ($this->arrayband as $band) {
//            $this->currentband=$band["name"]; // to know current where current band in!
// echo "<br>band name =>".$band["name"];
            switch($band["name"]) {


                case "title":
                //	echo "<br> porcessing title";
   ...
   $randomchartno=rand();
    ...
    public function title() {
        ...
        foreach ($this->arraytitle as $out) {
        	//  echo "<br> this title : out text => <pre>";print_r($out);
            switch($out["hidden_type"]) {
                case "field":
                	// echo "<br> display field<br><pre>";print_r($out);
                    $this->display($out,$this->arraytitle[0]["y_axis"],true);
                    break;
                default:
              //  	echo "<br> this title 2140 <pre>";
...
//		$this->pdf->SetY($checkpoint+$gheader);
	//	$this->pdf->MultiCell(200,10,"????",1);
      ...
      //                   echo $checkpoint." - JAsper L2633 <br/>";
...
//		$this->pdf->Cell(300,10,"$currentY+$gfootheight > $pageheight-$bottommargin-$footerheight",1);
 ...
 // if (trim($arraydata['type'])=="MultiCell" || $arraydata['type']=="SetXY" ) {
 //    print_r($arraydata);
     //echo "<br/>";

    // 	if (trim($arraydata['txt'])=="9. INSTRUCTIONS")
   // print_r($arraydata);echo "<br/>";
...
//    echo "<br> data type => ".$arraydata["type"]."-- L 2836 - <pre> ";print_r($arraydata);
...
//            $y_axis=$this->SubReportCheckPoint;
    ...
    elseif($arraydata["type"]=="MultiCell") {
    	 // echo "<br> data type => ".$arraydata["type"]."-- L 2863 - <pre> ";print_r($arraydata);
      if($fielddata==false) {
      //	echo "<br>check over flow / max height = $maxheight<pre>";print_r($arraydata);
        $this->checkoverflow($arraydata,$this->updatePageNo($arraydata["txt"]),'',$maxheight);
      }
      elseif($fielddata==true) {
    //  	echo "<br> field data is true";
    ...
    elseif($arraydata["type"]=="SetXY") {
    	//echo "<br> SET X -> ". ($arraydata["x"]+$this->arrayPageSetting["leftMargin"]);
    	// echo "<br> SET Y -> ".($arraydata["y"]+$y_axis);
    ...
    public function checkoverflowOLD($arraydata,$txt="",$maxheight=0) {
    ...
    public function checkoverflow($arraydata,$txt="",$maxheight=0) {
        ...
        if($this->print_expression_result==true) {
            ...
            elseif($arraydata["poverflow"]=="false" && $arraydata["soverflow"]=="false") {
							...
							if ($this->pdf->GetStringWidth($txt) > $arraydata["width"] ){
								// Long text -> Print in a Multicell
							  $this->pdf->MultiCell($arraydata["width"], $arraydata["height"], $this->formatText($txt, $arraydata["pattern"]), $arraydata["border"],
							      $arraydata["align"], $arraydata["fill"],1,'','',true,0,true,true,$maxheight);

							}
							else {
								// Short text -> Print in a Cell
							  $this->pdf->Cell($arraydata["width"], $arraydata["height"],$this->formatText($txt, $arraydata["pattern"]),
							      $arraydata["border"],"",$arraydata["align"],$arraydata["fill"],$arraydata["link"]);
    ...
    /* Analyzing data from DB to place in fields accordingly */
    public function analyse_expression($data,$isPrintRepeatedValue="true") {
      ...
      //  	echo "<br>txt out <pre>";print_r($out);
            $i++;
    ...
    //	echo "<br> print exrp <br><pre>";print_r($data);
